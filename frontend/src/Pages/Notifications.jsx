...
const Notifications = () => {
  ...
  const getIcon = (type, fuelCollectionId) => {
    // Si c'est une notification de carburant, utiliser l'icône de carburant
    if (fuelCollectionId) {
      return faGasPump;
    }
  ...
  const getColor = (type, fuelCollectionId) => {
    // Si c'est une notification de carburant, utiliser la couleur verte
    if (fuelCollectionId) {
  ...
  const getBackgroundColor = (type, read, fuelCollectionId) => {
    if (read) return 'bg-white hover:bg-gray-50';
    
    // Si c'est une notification de carburant, utiliser le style vert
    if (fuelCollectionId) {
      ...
      {serviceStatus && (
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg p-4 mb-6 border border-blue-200">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            ...
            <div>
              <span className="font-medium text-gray-700">Dernière vérification:</span>
              <span className="ml-2 text-gray-600">
                {serviceStatus.lastCheck ? formatTimestamp(serviceStatus.lastCheck) : 'Jamais'}
              </span>
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-lg overflow-hidden border border-gray-200">
        {notifications.length === 0 ? (
          <div className="p-12 text-center text-gray-500">
            ...
            <p className="text-sm text-gray-400">Les notifications d'entretien et de carburant apparaîtront ici automatiquement</p>
          </div>
        ) : (
            ...
            {notifications.map((notification) => (
              ...
              >
                <div className="flex items-start space-x-4">
                                  <div className={`flex-shrink-0 mt-1 ${getColor(notification.type, notification.fuelCollectionId)}`}>
                  <FontAwesomeIcon icon={getIcon(notification.type, notification.fuelCollectionId)} className="text-xl" />
                </div>
                    ...
                    <div className="flex justify-between items-start">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center space-x-6 text-xs text-gray-500">
                          ...
                          {/* Affichage conditionnel selon le type de notification */}
                          {notification.fuelCollectionId ? (
                            // Notification de carburant
                            <>
                              <span className="flex items-center space-x-1">
                                <FontAwesomeIcon icon={faGasPump} className="text-xs" />
                                <span>{notification.quantite}L</span>
                              </span>
                              <span className="flex items-center space-x-1">
                                <FontAwesomeIcon icon={faCheckCircle} className="text-xs" />
                                <span>{notification.chauffeur}</span>
                              </span>
                            </>
                          ) : (
                            ...
                            <>
                              <span className="flex items-center space-x-1">
                                ...
                                <span>{notificationService.getTypeLabel(notification.maintenanceType)}</span>
                              </span>
